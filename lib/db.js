...
export async function listExpenses() {
  ...
  return sql`SELECT id, amount, category, note, expense_date, affects_balance, created_at FROM expenses ORDER BY expense_date DESC, created_at DESC;`;
}
export async function createExpense({ amount, category, note, date, affectsBalance }) {
  await ensureTables(); const sql = getSql();
  const rows = await sql`INSERT INTO expenses (amount, category, note, expense_date, affects_balance)
    VALUES (${amount}, ${category}, ${note || ""}, ${date}, ${affectsBalance !== false}) RETURNING *;`;
  return rows[0];
}
export async function updateExpense(id, { amount, category, note, date, affectsBalance }) {
  ...
  const rows = await sql`UPDATE expenses SET amount=${amount}, category=${category}, note=${note || ""},
    expense_date=${date}, affects_balance=${affectsBalance !== false} WHERE id=${id} RETURNING *;`;
  return rows[0];
}
...
export async function createMovie({ title, date, ticketPrice, canteenPrice, companions, myTake, publicTake, note, affectsBalance, quantity }) {
  await ensureTables(); const sql = getSql();
  const rows = await sql`INSERT INTO movies (title, watched_date, ticket_price, canteen_price, companions, my_take, public_take, note, affects_balance, quantity)
    VALUES (${title}, ${date}, ${ticketPrice}, ${canteenPrice}, ${companions || ""}, ${myTake}, ${publicTake}, ${note || ""}, ${affectsBalance !== false}, ${quantity || 1}) RETURNING *;`;
  return rows[0];
}
export async function updateMovie(id, { title, date, ticketPrice, canteenPrice, companions, myTake, publicTake, note, affectsBalance, quantity }) {
  await ensureTables(); const sql = getSql();
  const rows = await sql`UPDATE movies SET title=${title}, watched_date=${date}, ticket_price=${ticketPrice}, canteen_price=${canteenPrice},
