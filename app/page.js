...
import { Plus, Trash2, ChevronLeft, ChevronRight, Wallet, X } from "lucide-react";
...
export default function Page() {
  ...
  const movieItems = useMemo(() => {
    ...
    for (const m of movies) {
      if (m.ticketPrice > 0) {
        items.push({ id: `movie-${m.id}-ticket`, amount: m.ticketPrice, category: MOVIE_CATEGORY.name, note: `${m.title} (ticket)`, date: m.date });
      }
      if (m.canteenPrice > 0) {
        items.push({ id: `movie-${m.id}-canteen`, amount: m.canteenPrice, category: MOVIE_CATEGORY.name, note: `${m.title} (canteen)`, date: m.date });
      }
    }
    return items;
  }, [movies]);
  ...
  const totalSpentAllTime = useMemo(() => allItems.reduce((s, x) => s + x.amount, 0), [allItems]);
  const balance = totalToppedUp - totalSpentAllTime;
  ...
  async function addTopup(e) {
    ...
    try {
      const res = await fetch("/api/balance", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ amount: amt, note: topupNote.trim(), date: topupDate }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
      ...
      setTopupError(err.message || "Couldn't add that. Try again.");
    } finally {
  ...
  return (
    <div style={{ maxWidth: 920, margin: "0 auto", padding: "40px 24px 64px" }}>
      ...
      <div style={{ background: "#F1ECDF", borderRadius: 6, padding: "18px 20px", marginBottom: 28 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: "#8a8477", marginTop: 4 }}>
              {fmt(totalToppedUp)} added in total · {fmt(totalSpentAllTime)} spent in total
            </div>
          </div>
          <button
            onClick={() => setShowTopup((v) => !v)}
            style={{
              background: showTopup ? "#EAE5D9" : "#241F1A",
              color: showTopup ? "#241F1A" : "#FBF8F2",
              border: "none",
              borderRadius: 3,
              padding: "8px 14px",
              fontSize: 13,
        ...
        {showTopup && (
          <form onSubmit={addTopup} style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap", alignItems: "flex-start" }}>
            <input
              type="number"
              inputMode="decimal"
              placeholder="Amount (₹)"
            ...
            <input
              ...
              style={{ width: 160 }}
            />
            <input type="date" value={topupDate} onChange={(e) => setTopupDate(e.target.value)} max={todayISO()} style={{ width: 150 }} />
            <button
              type="submit"
              ...
              style={{
                ...
                fontSize: 13,
                fontWeight: 500,
                cursor: topupSubmitting ? "default" : "pointer",
                opacity: topupSubmitting ? 0.6 : 1,
              }}
            >
              {topupSubmitting ? "Saving…" : "Add"}
            </button>
              ...
              <div key={t.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 12, color: "#5f5a4f", padding: "4px 0" }}>
                ...
                <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <button
                    ...
                    style={{ background: "none", border: "none", cursor: "pointer", color: "#C4BDAC", padding: 2, display: "flex" }}
