  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (type === "expenses") {
        ...
        const expense = await db.createExpense({ amount, category: body.category, note: body.note, date: body.date, affectsBalance: body.affectsBalance });
      ...
      if (type === "movies") {
        ...
        const movie = await db.createMovie({
          title: body.title.trim(), date: body.date, ticketPrice, canteenPrice,
    ...
    if (req.method === "PUT") {
      ...
      if (type === "expenses") {
        ...
        if (!body.category) return err(res, "Category is required");
        if (!body.date) return err(res, "Date is required");
        const expense = await db.updateExpense(numId, { amount, category: body.category, note: body.note, date: body.date, affectsBalance: body.affectsBalance });
